...
This paper does **not** claim that topological features alone beat traditional financial features at predicting cryptocurrency direction. The core claim is narrower and, we believe, more defensible:

> Persistent-homology summaries of multivariate price-volume sliding windows contain a **weak but statistically detectable** structural signal that (a) survives a block-permutation test against a shuffled-label null, (b) changes which signals a downstream classifier fires (expanding coverage by ~30% over a base-feature-only model at comparable per-signal accuracy), and (c) accounts for the largest share of permutation feature importance among the non-asset features in our pooled multi-asset model. Whether that signal converts into post-cost economic value depends on the prediction horizon, the execution venue, and the confidence threshold; we present a conservative offline projection but make no live-trading claim.

The remainder of the paper documents the methodology, the validation regime, the ablation that informs this claim, and the conditions under which the strategy plausibly clears realistic transaction costs.
...
Our results are **consistent with** persistent-homology features carrying a weak but statistically detectable directional signal in cryptocurrency markets, with the apparent signal concentrated in less liquid mid-cap altcoins. The cross-horizon consistency (5/6 horizons clearing the Wilson lower-bound test) and per-asset heterogeneity (gradient from ADA at 76.75% to BTC at 48.48%) are *compatible with* a market-efficiency reading: TDA summaries appear to track dynamical structure in price-volume manifolds, and that structure is most exploitable where market efficiency is weakest. We deliberately frame this conclusion descriptively rather than causally — the ablation in §3.3 shows that base microstructure features alone reach 64.63% direction accuracy, exceeding the Base+TDA combination's 62.14%, and the marginal contribution of TDA is best characterized as *coverage expansion* (more signals fired) rather than improved per-signal accuracy.

Three mechanisms are *consistent with* the patterns we observe in the TDA features, though we make no claim to have identified which (if any) is operative. First, persistent homology in high-volatility regimes is sensitive to *regime transitions* — moments when local manifold geometry changes — and such transitions can precede directional moves. Second, the $L^p$-norm and entropy summaries respond to the *number* of distinct regimes the system is currently transitioning between, which may serve as a rough proxy for trader uncertainty. Third, by pooling across seven assets, the classifier is forced to learn *transferable* structural patterns rather than asset-specific quirks; in our experiments, single-asset variants of the same architecture failed to beat chance, while the pooled variant succeeded. We present these as *plausible* mechanisms for further investigation, not as established explanations.
